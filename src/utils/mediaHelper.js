...
export function getRecordImage(record) {
  ...
  if (Array.isArray(record.images) && record.images.length > 0) {
    ...
    if (typeof first === 'string' && !first.includes('wsrv.nl')) return first
    if (first && first.url && !first.url.includes('wsrv.nl')) return first.url
  }
...
export function getRecordGalleryImages(record) {
  ...
  if (Array.isArray(record.images) && record.images.length > 0) {
    return record.images
      ...
      .filter(Boolean)
      .filter(img => !img.includes('wsrv.nl')) // filter out broken wsrv proxies
  }
  return []
}
...
export function getCategoryVisualTheme(categorySlug, recordType = 'heritage') {
  const themeMap = {
    ...
    museums: { bg: 'linear-gradient(135deg, #1A237E 0%, #303F9F 50%, #5C6BC0 100%)', accent: '#C5CAE9', icon: 'museum', label: 'Museum' },
    'historic-places': { bg: 'linear-gradient(135deg, #4A2C1E 0%, #7A331C 50%, #B84B2A 100%)', accent: '#FFCCBC', icon: 'location_city', label: 'Historic Place' },
    monuments: { bg: 'linear-gradient(135deg, #3E2723 0%, #7A331C 50%, #B84B2A 100%)', accent: '#FFCCBC', icon: 'account_balance', label: 'Monument' },

    // Culture Categories
    'classical-folk-dance': { bg: 'linear-gradient(135deg, #4A148C 0%, #7B1FA2 50%, #AB47BC 100%)', accent: '#F3E5F5', icon: 'self_improvement', label: 'Dance & Performance' },
    music: { bg: 'linear-gradient(135deg, #311B92 0%, #4527A0 50%, #673AB7 100%)', accent: '#D1C4E9', icon: 'music_note', label: 'Musical Heritage' },
    festivals: { bg: 'linear-gradient(135deg, #BF360C 0%, #E65100 50%, #F57C00 100%)', accent: '#FFE0B2', icon: 'celebration', label: 'Festival & Fair' },
    handicrafts: { bg: 'linear-gradient(135deg, #004D40 0%, #00695C 50%, #00897B 100%)', accent: '#B2DFDB', icon: 'handyman', label: 'Handicraft' },
    'traditional-arts': { bg: 'linear-gradient(135deg, #1A237E 0%, #283593 50%, #3949AB 100%)', accent: '#C5CAE9', icon: 'palette', label: 'Traditional Art' },
    'clothing-textiles': { bg: 'linear-gradient(135deg, #880E4F 0%, #AD1457 50%, #D81B60 100%)', accent: '#F8BBD0', icon: 'checkroom', label: 'Textiles & Handloom' },
    'regional-cuisine': { bg: 'linear-gradient(135deg, #E65100 0%, #EF6C00 50%, #F57C00 100%)', accent: '#FFE0B2', icon: 'restaurant', label: 'Culinary Heritage' },
  ...
  if (recordType === 'heritage') return { bg: 'linear-gradient(135deg, #4A2C1E 0%, #7A331C 50%, #B84B2A 100%)', accent: '#FFCCBC', icon: 'account_balance', label: 'Heritage' }
  return { bg: 'linear-gradient(135deg, #3C245C 0%, #2C3E73 50%, #6C4BA0 100%)', accent: '#E1BEE7', icon: 'palette', label: 'Culture' }
}
