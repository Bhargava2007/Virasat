...
console.log(
  'Missing Core Fields (Name/State/Category/Desc):',
  missingFields.name +
    missingFields.state +
...
culturalTrails.forEach(t => {
  if (
    !t.coverImage ||
    !fs.existsSync(join(projectDir, 'public/images', t.coverImage))
  ) {
...
console.log(
  'Total Mapped Records (with precise coordinates):',
...
console.log(
  `All ${allRecords.length} record IDs are unique and preserved. Passport + Saved Items compatibility: PASSED.`
)
